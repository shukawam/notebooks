...
def init_dls_dp_client(config, service_endpoint):
    dls_client = DataLabelingClient(
        config=config, service_endpoint=service_endpoint)
    return dls_client
...
def process_annotation_and_remove_label(logger, annotation_id):
    ...
    if response.status == 200:
        ...
        if not newLabelList:
            ...
            new_entity = [GenericEntity(
                entity_type=entity_type, extended_metadata=extended_metadata, labels=newLabelList)]
...
def update_annotation(annotation, entity):
    update_annotation_details_obj = UpdateAnnotationDetails(
        defined_tags=annotation.defined_tags, entities=entity, freeform_tags=annotation.freeform_tags)
    try:
        response = dls_dp_client.update_annotation(
            annotation_id=annotation.id, update_annotation_details=update_annotation_details_obj)
        global success_count
        success_count += 1
    except Exception as error:
        print(error)
...
def main():
    ...
    if response.status == 200:
        ...
        while True:
            annotation_response = dls_dp_client.list_annotations(
                compartment_id=compartment_id, dataset_id=DATASET_ID, lifecycle_state="ACTIVE", page=page)
            annotation_data = json.loads(str(annotation_response.data))
            annotation_ids = [annotation["id"]
                              for annotation in annotation_data["items"]]

            with ThreadPoolExecutor(NO_OF_PROCESSORS) as executor:
                futures = [executor.submit(
                    process_annotation_and_remove_label, logger, annotation_id) for annotation_id in annotation_ids]
            wait(futures)
