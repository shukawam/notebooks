...
def init_object_storage_client():
    ...
    object_storage_client = oci.object_storage.ObjectStorageClient(
        config=config, service_endpoint=SERVICE_ENDPOINT_OBJECT_STORAGE, retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY)
...
def put_object(logger, file):
    ...
    if os.path.isfile(file_path):
        ...
        if OBJECT_STORAGE_PREFIX == "":
            ...
            bucket_name=OBJECT_STORAGE_BUCKET_NAME,
            content_type=mimetype,
            object_name=OBJECT_STORAGE_PREFIX + file,
            put_object_body=file_bytes)
        ...
        logger.info(
            f'Failed to upload {file}, Response header : {put_object_response.headers}')
        return False
...
def main():
    ...
    logger.info(
        "...........starting to upload files to object storage.............")
    start = time.perf_counter()
    with ThreadPoolExecutor(NO_OF_PROCESSORS) as executor:
        futures = [executor.submit(put_object, logger, file)
