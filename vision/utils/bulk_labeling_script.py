...
def dls_list_records(compartment_id, page):
    ...
    names = [dls_dataset_record["name"]
             for dls_dataset_record in data["items"]]
...
def dls_create_annotation(label, record_id, compartment_id):
    ...
    try:
        anno_response = dls_dp_client.create_annotation(
            create_annotation_details=create_annotation_details_obj)
    except Exception as error:
        anno_response = error
        print(anno_response)
...
def first_letter(name, record_id, compartment_id, logger):
    ...
    if label:
        logger.info("labeling record id: " + str(record_id) +
                    "of name " + str(name) + " with label " + str(label))
        dls_create_annotation(
            label=label, record_id=record_id, compartment_id=compartment_id)
    else:
        logger.info("No label match for record " +
                    str(name) + " with id: " + str(record_id))
...
def custom_label_match(name, record_id, compartment_id, logger):
    ...
    if label_var != "":
        ...
        logger.info("labeling record id: " + str(record_id) +
                    "of name " + str(name) + " with label " + str(label_lst))
        dls_create_annotation(
            label=label_lst, record_id=record_id, compartment_id=compartment_id)
    else:
        logger.info("No label match for record " +
                    str(name) + " with id: " + str(record_id))
...
def label_record(name, id, labeling_algorithm, compartment_id, logger):
    ...
    if labeling_algorithm == "FIRST_REGEX_MATCH":
        logger.info(
            "labeling algorithm FIRST_REGEX_MATCH for record_id: " + str(id))
        first_match(name=name, record_id=id,
                    compartment_id=compartment_id, logger=logger)
    elif labeling_algorithm == "FIRST_LETTER_MATCH":
        logger.info(
            "labeling algorithm FIRST_LETTER_MATCH for record_id: " + str(id))
        first_letter(name=name, record_id=id,
                     compartment_id=compartment_id, logger=logger)
    elif labeling_algorithm == "CUSTOM_LABELS_MATCH":
        logger.info(
            "labeling algorithm CUSTOM_LABELS_MATCH for record_id: " + str(id))
...
def bounding_box_annotation(row, compartment_id, logger):
    ...
    for row_ent in row:
        ...
        for i in range(4):
            normalized_vector_obj = NormalizedVertex(
                x=row_ent[i + 1], y=row_ent[i + 5])
            normalized_vector_obj_lst.append(normalized_vector_obj)

        bounding_polygon_obj = BoundingPolygon(
            normalized_vertices=normalized_vector_obj_lst)
...
def main():
    ...
    if response.status == 200:
        ...
        if ANNOTATION_TYPE == "BOUNDING_BOX":
            ...
            rows = df.groupby('record_id').apply(
                lambda x: x.values.tolist()).tolist()[:]
            pool.starmap(bounding_box_annotation, zip(
                rows, repeat(compartment_id), repeat(logger)))
            pool.close()
            end = time.perf_counter()
            print(f'Finished in {round(end - start, 2)} second(s)')
        elif ANNOTATION_TYPE == "CLASSIFICATION":
            while True:
                names, ids, num_records, page = dls_list_records(
                    compartment_id=compartment_id, page=page)
                pool.starmap(label_record, zip(names, ids, repeat(
                    LABELING_ALGORITHM), repeat(compartment_id), repeat(logger)))
                if not page:
                    break
            pool.close()
            end = time.perf_counter()
